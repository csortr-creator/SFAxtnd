...
class HTTPClient : Closeable {
    ...
    fun getString(url: String): String {
        ...
        val model =
            if (rawModel.startsWith(manufacturer, ignoreCase = true)) {
                rawModel
            } else {
        ...
        val buildId = Build.ID.ifEmpty { "UNKNOWN" }

        val userAgentStr =
            "sing-box/1.14.0 SFAxtnd/0.0.8 " +
                "(Linux; Android $androidVer; $model Build/$buildId) " +
                "HWID/$hwid"
    ...
    private fun getOrCreateHwid(url: String): String {
        ...
        val urlKey = try {
            ...
            val hash = digest.digest(
                normalizedUrl.toByteArray(StandardCharsets.UTF_8)
            )
        ...
        if (context != null) {
            ...
            val newHwid = UUID.randomUUID()
                .toString()
                .replace("-", "")
                .take(16)

            prefs.edit()
                .putString(urlKey, newHwid)
                .apply()
    ...
    private fun getApplicationContext(): Context? {
        return try {
            val appClass =
                Class.forName("io.nekohasekai.sfa.Application")

            val field =
                appClass.getDeclaredField("application")
        ...
        if (
            ...
            (
                contentToParse.contains("\"outbounds\"") ||
                    ...
                    contentToParse.contains("\"inbounds\"")
                )
        ) {
        ...
        if (contentToParse.startsWith("[")) {
            try {
                ...
                for (i in 0 until jsonArray.length()) {
                    val obj =
                        jsonArray.optJSONObject(i) ?: continue
    ...
    private fun migrateDns(root: JSONObject) {
        val dns = root.optJSONObject("dns") ?: return

        dns.remove("independent_cache")

        if (dns.has("address_strategy")) {
            val strategy = dns.remove("address_strategy")

            if (!dns.has("strategy")) {
                dns.put("strategy", strategy)
            }
        }
        ...
        for (i in 0 until servers.length()) {
            val server =
                servers.optJSONObject(i) ?: continue
    ...
    private fun migrateInbounds(root: JSONObject) {
        val inbounds = root.optJSONArray("inbounds") ?: return

        for (i in 0 until inbounds.length()) {
            val inbound =
                inbounds.optJSONObject(i) ?: continue

            /*
             * sing-box 1.13+:
             * sniffing больше не настраивается
             * внутри inbound.
             *
             * sniff выполняется route action:
             * {
             *   "action": "sniff"
             * }
             */
            ...
            /*
             * Старые поля sniffing из конфигов
             * SFA/Hiddify/Xray.
             */
            ...
            /*
             * Legacy domain strategy inbound.
             */
            ...
            if (inbound.optString("type") == "tun") {
                ...
                /*
                 * Старые sing-box TUN поля,
                 * которые могут конфликтовать
                 * с современным форматом.
                 */
    ...
    private fun migrateOutbounds(root: JSONObject) {
        val outbounds =
            root.optJSONArray("outbounds") ?: return

        val cleanedOutbounds = JSONArray()

        for (i in 0 until outbounds.length()) {
            val outbound =
                outbounds.optJSONObject(i) ?: continue
    ...
    private fun migrateRoute(root: JSONObject) {
        ...
        /*
         * В sing-box 1.13+ sniffing должен быть
         * route action, а не полем inbound.
         */
        ...
        for (i in 0 until oldRules.length()) {
            val rule =
                oldRules.optJSONObject(i) ?: continue

            /*
             * Старый sniff action не дублируем.
             */
...
private fun tryDecodeBase64(text: String): String {
    ...
    val nonCommentLines = text.lines()
        .filter {
            val t = it.trim()
            t.isNotEmpty() &&
                !t.startsWith("//") &&
...
private fun parseVless(line: String): JSONObject? {
    val rawTag =
        if (line.contains("#")) line.substringAfter("#") else ""
    ...
    private fun parseVmess(line: String): JSONObject? {
        ...
        if (server.isBlank() || uuid.isBlank()) {
            return null
        }
        ...
        outbound.put(
            "security",
            vmessJson.optString("scy", "auto")
        )

        val tlsEnabled =
            vmessJson.optString("tls")
                .equals("tls", ignoreCase = true)
        ...
        if (net == "ws" || net == "grpc") {
            ...
            when (net) {
                ...
                "grpc" -> {
                    val serviceName = vmessJson.optString("path")
    ...
    private fun parseTrojan(line: String): JSONObject? {
        ...
        if (password.isBlank() || hostPort.isBlank()) {
            return null
        }
    ...
    private fun buildSingBoxConfig(
        ...
        for (node in validNodes) {
            ...
            if (
                ...
                if (
                    normalizedTag.contains("обход") ||
                    normalizedTag.contains("white list") ||
                    normalizedTag.contains("whitelist")
                ) {
